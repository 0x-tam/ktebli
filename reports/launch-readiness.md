...
| 6 — engineering | P0 refund-blocking constraint regression fixed; invariant-2 clearance enforced both sides (DB-atomic single-use); F1/F3/F5 closed fail-closed; hold-class notifications with DRAFT wordings; strand release; crawl-starvation hold; per-stage cost accounting; resumable generation (unproven on deployed runtime). | $0 |
| 6 — benchmark | _[pending WS6-bench]_ | _[≤$8]_ |
| 6.5 — adversarial | _[pending]_ | _[$0–small]_ |
